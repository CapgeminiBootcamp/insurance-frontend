...
class ViewInsurance extends React.Component {
    ...
    updateInsurance(id) {
        this.props.history.push(`/updateInsurance/${id}`);
        window.location.reload();
    }
    render() {
        return (
                            ...
                            this.state.insurances.map(
                                insurance =>
                                    <tr key={insurance.insuranceId}>
                                        ...
                                        <td>
                                            <button onClick={() => this.updateInsurance(insurance.insuranceId)} className="btn btn-outline-warning" style={{ marginLeft: "8px" }} ><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
